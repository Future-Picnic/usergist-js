...
test('public commit contains only selected SDK files, no private parents, and is reusable for identical SDK content', () => {
  ...
  assert.deepEqual(paths, ['.gitignore', 'package.json', 'packages/sdk-core/src/index.ts', 'pnpm-workspace.yaml'])
  assert.match(git('show', `${first}:.gitignore`), /node_modules\//)
  assert.match(git('show', `${first}:.gitignore`), /\.ci\//)
